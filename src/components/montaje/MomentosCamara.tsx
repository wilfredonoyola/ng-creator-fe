...
export function MomentosCamara({
  ...
  /** Si se esta mandando otra toma general en lugar de la que ya hay. */
  ...
  /**
   * Saca la toma general, no los momentos.
   *
   * Antes borraba las dos cosas y habia que confirmarlo: perder seis momentos
   * configurados a mano por querer otra grabacion era el peor resultado posible.
   * Con los momentos definiendose primero, ya ni siquiera se parecen a algo que
   * se borre junto con un archivo.
   */
  function quitar() {
    onCamara(camara ? { ...camara, origenStoragePath: undefined } : null);
  ...
  /**
   * Los momentos existen antes que la grabacion, asi que hace falta un lugar
   * donde vivan la posicion y el tamaño desde el primero: son ajustes de la
   * camara aunque todavia no haya archivo. Por eso `origenStoragePath` quedo
   * opcional en el contrato.
   */
  function asegurarCamara() {
    if (camara) return;
    onCamara({
      posicion: "ABAJO_DERECHA",
      tamano: 0.32,
  ...
  const sinGrabar = momentos.filter(
    (m) => !m.origenStoragePath && !camara?.origenStoragePath,
  ).length;
  const compartida = momentos.filter((m) => !m.origenStoragePath).length;
  ...
  return (
    <section className="rounded-2xl border border-white/10 bg-white/5 p-4">
      ...
      {/* PRIMERO los momentos, DESPUES la grabacion.
          Al reves —que es como estaba— los momentos no existian hasta subir un
          video, asi que la primera toma salia siempre a ciegas: no habia forma
          de saber que decir ni cuanto, porque eso es justo lo que definen los
          momentos. Se terminaba grabando cualquier cosa para desbloquear el
          panel, y reemplazandola despues. */}
      <div className="mt-2 space-y-3">
        ...
        {/* La grabacion general: la que se reparten los que no traen la suya.
            Aparece recien cuando hay algo que grabar, porque antes no hay
            ninguna pregunta que este panel pueda contestar. */}
        {compartida > 0 && (
          <div className="border-t border-white/10 pt-3">
            {camara?.origenStoragePath && !reemplazando ? (
